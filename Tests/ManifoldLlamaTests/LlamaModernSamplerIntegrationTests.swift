...
final class LlamaModernSamplerIntegrationTests: XCTestCase {
  ...
  func test_xtc_sameSeed_isDeterministic() async throws {
    ...
    let outputA = try await collectTokens(
      backend: backend, prompt: "Tell me a fact:", config: config)
    backend.resetConversation()
    let outputB = try await collectTokens(
      backend: backend, prompt: "Tell me a fact:", config: config)
  ...
  func test_mirostatV2_sameSeed_isDeterministic() async throws {
    ...
    XCTAssertEqual(
      outputA, outputB, "Same seed + same Mirostat config must produce identical streams")
  }
  ...
  func test_differentSeeds_divergeOverLongStream() async throws {
    ...
    for (seedA, seedB) in seedPairs {
      ...
      let outputA = try await collectTokens(
        backend: backend, prompt: "Tell me a story:", config: configA)
      backend.resetConversation()
      let outputB = try await collectTokens(
        backend: backend, prompt: "Tell me a story:", config: configB)
      backend.resetConversation()
    ...
    XCTFail(
      "All \(seedPairs.count) distinct seed pairs produced identical \(longStreamTokens)-token "
