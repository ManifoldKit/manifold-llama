...
final class LlamaModelLoaderKVQuantizationTests: XCTestCase {
  ...
  func test_loadModel_eachKVQuantization_loadsWorkingContext() async throws {
    ...
    guard let modelURL = HardwareRequirements.findGGUFModel() else {
      throw XCTSkip(
        "No chat GGUF available. Set LLAMA_TEST_MODEL=<path> or place a `.gguf` in ~/Documents/Models/ to run the KV-quantization load tests."
      )
    }
  ...
  func test_contextCreationFailure_codeIsDistinctFromModelLoadFailure() {
    guard
      case InferenceError.modelLoadFailed(let underlying) =
        LlamaModelLoader.contextCreationFailure(effectiveContextSize: 8192)
    else {
      return XCTFail("Expected .modelLoadFailed")
    }
    XCTAssertNotEqual(
      (underlying as NSError).code, -1,
  ...
  func test_contextCreationFailure_messageNamesRequestedContextSize() {
    guard
      case InferenceError.modelLoadFailed(let underlying) =
        LlamaModelLoader.contextCreationFailure(effectiveContextSize: 12345)
    else {
      return XCTFail("Expected .modelLoadFailed")
    }
