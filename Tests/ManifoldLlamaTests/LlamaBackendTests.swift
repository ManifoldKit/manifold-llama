...
final class LlamaBackendTests: XCTestCase {
  ...
  func test_loadModel_fromPlan_clampRespected_noMetalHostMallocFailure() async throws {
    ...
    let plan = ModelLoadPlan.compute(inputs: inputs)
    XCTAssertLessThan(
      plan.effectiveContextSize, inputs.requestedContextSize,
  ...
  func test_tokenCount_withoutLoadedModel_fallsBackToHeuristic() {
    ...
    XCTAssertFalse(backend.isModelLoaded)
    XCTAssertEqual(
      backend.tokenCount("hello world"), 2,
  ...
  func test_countTokens_idempotent_withLoadedModel() async throws {
    guard let modelURL = HardwareRequirements.findGGUFModel() else {
      throw XCTSkip(
        "No GGUF model on disk. Set LLAMA_TEST_MODEL=<path> or place a `.gguf` in ~/Documents/Models/ to run this test."
      )
    }
  ...
  func test_fixture_decodeErrorPath_publicErrorContractForGeneratePreconditions_scaffold521()
    async throws
  {
    let backend = LlamaBackend()
  ...
  func test_fixture_stopGeneration_midDecode_isGeneratingFallsFalseWithinBudget_regression522()
    async throws
  {
    guard let modelURL = HardwareRequirements.findGGUFModel() else {
      throw XCTSkip(
    ...
    XCTAssertTrue(
      sawToken,
      "Expected at least three tokens (visible or thinking) before stopping — generation never reached the decode loop"
    )
  ...
  func test_tokenCount_noVocab_matchesHeuristicTokenizer() {
    ...
    XCTAssertEqual(
      backend.tokenCount(short), HeuristicTokenizer.tokenCount(short),
      "short-string fallback must match shared heuristic")
    XCTAssertEqual(
      backend.tokenCount(long), HeuristicTokenizer.tokenCount(long),
  ...
  func test_generate_withImageParts_throwsInferenceFailure() {
    ...
    XCTAssertThrowsError(
      ...
      guard let inferenceError = error as? InferenceError,
        case .inferenceFailure(let message) = inferenceError
      else {
        XCTFail("Expected InferenceError.inferenceFailure, got \(error)")
        return
      }
      XCTAssertTrue(
        message.contains("multimodal image inference is not yet available"),
        "expected image-guard message, got: \(message)")
      XCTAssertTrue(
        message.contains("clip.h"),
