...
final class EvalCLIArgumentParsingTests: XCTestCase {
  ...
  private func runCLI(_ arguments: [String]) throws -> (
    ...
    guard FileManager.default.fileExists(atPath: binary.path) else {
      throw XCTSkip(
        "manifold-llama-eval binary not found at \(binary.path) — expected `swift build` to have produced it before `swift test` (CI always builds before testing)."
      )
    }
  ...
  func test_topK_rejectsNegativeValue() throws {
    ...
    XCTAssertEqual(
      result.exitCode, 2,
      "negative --top-k must be a bad-argument exit (2); stderr: \(result.stderr)")
    XCTAssertTrue(
      result.stderr.contains("--top-k"),
      "stderr should name the offending flag; got: \(result.stderr)")
  }
  ...
  func test_topK_rejectsValueAboveInt32Max() throws {
    ...
    XCTAssertEqual(
      result.exitCode, 2,
      "out-of-Int32-range --top-k must be a bad-argument exit (2), not a crash; stderr: \(result.stderr)"
    )
    XCTAssertTrue(result.stderr.contains("--top-k"), "got: \(result.stderr)")
  }
  ...
  func test_topK_acceptsZeroAndPositiveValues() throws {
    ...
    for value in ["0", "40", "1"] {
      ...
      XCTAssertEqual(result.exitCode, 2)
      XCTAssertFalse(
        result.stderr.contains("--top-k requires"),
  ...
  func test_repeatPenalty_rejectsZero() throws {
    ...
    XCTAssertEqual(
      result.exitCode, 2,
      "non-positive --repeat-penalty must be a bad-argument exit (2); stderr: \(result.stderr)")
  ...
  func test_repeatPenalty_acceptsPositiveValues() throws {
    for value in ["1.0", "1.1", "0.5"] {
      ...
      XCTAssertEqual(result.exitCode, 2)
      XCTAssertFalse(
        result.stderr.contains("--repeat-penalty requires"),
  ...
  func test_help_documentsNewFlags() throws {
    ...
    XCTAssertTrue(
      result.stdout.contains("--repeat-penalty"), "usage text must document --repeat-penalty")
  }
}
