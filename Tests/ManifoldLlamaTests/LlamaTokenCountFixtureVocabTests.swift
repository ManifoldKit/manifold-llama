...
final class LlamaTokenCountFixtureVocabTests: XCTestCase {
  ...
  func test_tokenCount_plainText_matchesByteFallback() async throws {
    ...
    let plain = "helo"
    XCTAssertEqual(
      backend.tokenCount(plain), plain.utf8.count,
  ...
  func test_tokenCount_withoutVocab_usesHeuristic() async throws {
    ...
    XCTAssertEqual(
      backend.tokenCount(text), HeuristicTokenizer.tokenCount(text),
...
private final class FixtureVocab {
  ...
  static func make() throws -> FixtureVocab {
    ...
    types.append(3)  // multi-char CONTROL token
    ...
    scores.withUnsafeBufferPointer { buf in
      gguf_set_arr_data(
        ctx, "tokenizer.ggml.scores", GGUF_TYPE_FLOAT32,
        buf.baseAddress, tokens.count)
    }
    types.withUnsafeBufferPointer { buf in
      gguf_set_arr_data(
        ctx, "tokenizer.ggml.token_type", GGUF_TYPE_INT32,
        buf.baseAddress, tokens.count)
    }
...
extension Array where Element == String {
  ...
  /// duration of `body`.
  fileprivate func withUnsafeStrArray<R>(_ body: (UnsafeMutablePointer<UnsafePointer<CChar>?>) -> R)
    -> R
  {
