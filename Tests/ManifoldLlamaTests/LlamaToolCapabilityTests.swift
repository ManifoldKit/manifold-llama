...
final class LlamaToolCapabilityTests: XCTestCase {
  ...
  func test_capabilities_supportsToolCalling_isTrue() {
    ...
    XCTAssertTrue(
      backend.capabilities.supportsToolCalling,
  ...
  func test_chatML_withTools_ignoresToolsParameter() {
    ...
    let withTools = PromptTemplate.chatML.format(
      messages: [(role: "user", content: "hi")], systemPrompt: nil, tools: [tool])
    let withoutTools = PromptTemplate.chatML.format(
      messages: [(role: "user", content: "hi")], systemPrompt: nil, tools: [])
    XCTAssertEqual(
