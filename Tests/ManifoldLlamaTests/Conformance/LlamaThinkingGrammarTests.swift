...
final class LlamaThinkingGrammarTests: XCTestCase {
  ...
  private func loadQwenBackend() async throws -> LlamaBackend {
    guard let modelURL = HardwareRequirements.findGGUFModel(nameContains: "qwen"),
      modelURL.path.lowercased().contains("qwen")
    else {
      throw XCTSkip(
        "No GGUF on disk for family 'qwen'. "
  ...
  func test_grammar_thinkingEnabled_permissiveDuringThinking_strictAfter() async throws {
    ...
    guard backend.manifest?.supportsThinking == true else {
      print(
        "[thinking-grammar][qwen] loaded GGUF advertised no thinking markers "
      ...
      throw XCTSkip(
        "Loaded GGUF is not thinking-capable; the #1595 phase gate cannot be exercised.")
    }
    ...
    if !reasoning.isEmpty {
      ...
      let reasoningLooksConstrained =
        ["yes", "no"].contains(
          reasoning.trimmingCharacters(in: .whitespacesAndNewlines).lowercased())
      XCTAssertFalse(
        reasoningLooksConstrained,
      ...
      print(
        "[thinking-grammar][qwen] permissive phase confirmed: reasoning surfaced as "
          + "\(reasoning.count) chars of free .thinkingToken text")
    } else {
      print(
        "[thinking-grammar][qwen] no .thinkingToken surfaced for this prompt "
    ...
    if sawThinkingCompleted {
      ...
      print(
        "[thinking-grammar][qwen] strict tail confirmed: post-</think> output \(trimmed.debugDescription)"
      )
    } else {
      ...
      print(
        "[thinking-grammar][qwen] </think> did not close within \(config.maxOutputTokens ?? -1) tokens "
