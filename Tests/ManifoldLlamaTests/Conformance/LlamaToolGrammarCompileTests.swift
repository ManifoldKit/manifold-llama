...
final class LlamaToolGrammarCompileTests: XCTestCase {
  ...
  private func objectSchema(_ props: [(String, JSONSchemaValue)], required: [String])
    -> JSONSchemaValue
  {
  ...
  private func loadGrammarCapableBackend() async throws -> LlamaBackend {
    ...
    guard let modelURL = resolved else {
      throw XCTSkip(
        "No grammar-capable GGUF on disk (looked for fragments \(fragments)). "
  ...
  private func grammarConfig(_ grammar: String, tools: [ToolDefinition], maxTokens: Int = 96)
    -> GenerationConfig
  {
  ...
  private func parseEnvelope(_ text: String, _ label: String) -> [String: Any]? {
    ...
    guard let data = trimmed.data(using: .utf8),
      ...
      XCTFail(
        "\(label): grammar-constrained output must parse as a JSON object; got \(trimmed.debugDescription)"
      )
      return nil
    }
    return obj
  ...
  func test_compile_multiToolUnion_constrainsToOneTool() async throws {
    ...
    let grammar = try XCTUnwrap(
      builder.buildGrammar(for: tools),
    ...
    XCTAssertTrue(
      ["get_weather", "set_timer"].contains(name ?? ""),
