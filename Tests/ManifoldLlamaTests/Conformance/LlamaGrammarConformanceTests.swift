...
final class LlamaGrammarConformanceTests: XCTestCase {
  ...
  static let families: [GrammarFamily] = [
    GrammarFamily(
      id: "llama", nameFragment: "llama", expectsGrammarSupport: true, isThinkingCapable: false,
      toolCallDialect: .hermes),
    GrammarFamily(
      id: "qwen", nameFragment: "qwen", expectsGrammarSupport: true, isThinkingCapable: true,
      toolCallDialect: .qwenJSON),
    GrammarFamily(
      id: "mistral", nameFragment: "mistral", expectsGrammarSupport: true, isThinkingCapable: false,
      toolCallDialect: .generic),
    GrammarFamily(
      id: "gemma", nameFragment: "gemma", expectsGrammarSupport: false, isThinkingCapable: false,
      toolCallDialect: .gemma4),
    GrammarFamily(
      id: "phi", nameFragment: "phi", expectsGrammarSupport: true, isThinkingCapable: false,
      toolCallDialect: .generic),
  ]
  ...
  private func drainTokens(_ stream: GenerationStream, _ backend: LlamaBackend) async throws
    -> String
  {
    var text = ""
  ...
  private func waitForGeneratingFalse(_ backend: LlamaBackend) async throws {
    ...
    XCTAssertFalse(
      backend.isGenerating, "isGenerating must settle false before the next generate call")
  }
  ...
  private func loadBackend(for family: GrammarFamily) async throws -> LlamaBackend {
    ...
    guard let modelURL = HardwareRequirements.findGGUFModel(nameContains: family.nameFragment),
      modelURL.path.lowercased().contains(family.nameFragment.lowercased())
    else {
      throw XCTSkip(
        "No GGUF on disk for family '\(family.id)' (fragment: '\(family.nameFragment)'). "
  ...
  private func runC1(_ family: GrammarFamily, _ backend: LlamaBackend) async throws {
    let stream = try backend.generate(
      prompt: "Give me a random number.", systemPrompt: nil,
      config: grammarConfig(GrammarFixtures.digits, maxTokens: 16))
    let text = try await drainTokens(stream, backend)
    XCTAssertFalse(
      text.isEmpty, "[\(family.id)] C1: grammar-constrained generation must emit at least one token"
    )
    XCTAssertTrue(
  ...
  private func runC2(_ family: GrammarFamily, _ backend: LlamaBackend) async throws {
    let stream = try backend.generate(
      prompt: "Report the weather as JSON.", systemPrompt: nil,
      config: grammarConfig(GrammarFixtures.jsonObject, maxTokens: 64))
    let text = try await drainTokens(stream, backend)
    XCTAssertTrue(
      GrammarFixtures.isJSONObject(text, requiredKeys: GrammarFixtures.jsonObjectRequiredKeys),
  ...
  private func runC3(_ family: GrammarFamily, _ backend: LlamaBackend) async throws {
    let stream = try backend.generate(
      prompt: "Answer yes or no: is the sky blue?", systemPrompt: nil,
      config: grammarConfig(GrammarFixtures.alternation, maxTokens: 8))
    let text = try await drainTokens(stream, backend).trimmingCharacters(
      in: .whitespacesAndNewlines)
    XCTAssertTrue(
      ["yes", "no"].contains(text),
  ...
  private func runC4(_ family: GrammarFamily, _ backend: LlamaBackend) async throws {
    ...
    if plain == spaced {
      print(
        "[grammar-conformance][\(family.id)] C4 branch AGREEMENT: both variants → \(plain.debugDescription)"
      )
    } else {
      print(
        "[grammar-conformance][\(family.id)] C4 branch DIVERGENCE (model-intrinsic): "
  ...
  private func runC5(_ family: GrammarFamily, _ backend: LlamaBackend) async throws {
    ...
    let stream = try backend.generate(
      prompt: "Call the get_weather tool.", systemPrompt: nil,
      config: grammarConfig(GrammarFixtures.toolCallEnvelope, maxTokens: 64))
    let text = try await drainTokens(stream, backend).trimmingCharacters(
      in: .whitespacesAndNewlines)

    guard let open = text.range(of: "<tool_call>"),
      let close = text.range(of: "</tool_call>")
    else {
      XCTFail(
        "[\(family.id)] C5: grammar-constrained output must contain a <tool_call>…</tool_call> envelope; "
          + "got \(text.debugDescription)")
      return
    }
    let body = String(text[open.upperBound..<close.lowerBound]).trimmingCharacters(
      in: .whitespacesAndNewlines)
    guard let data = body.data(using: .utf8),
      let obj = (try? JSONSerialization.jsonObject(with: data)) as? [String: Any]
    else {
      XCTFail(
        "[\(family.id)] C5: tool-call envelope body must be a JSON object; got \(body.debugDescription)"
      )
      return
    }
    XCTAssertNotNil(
  ...
  private func runThinking(_ family: GrammarFamily, _ backend: LlamaBackend) async throws {
    ...
    guard backend.manifest?.supportsThinking == true else {
      print(
        "[grammar-conformance][\(family.id)] thinking: loaded GGUF advertised no thinking "
    ...
    if sawThinkingCompleted {
      ...
      XCTAssertTrue(
        ["yes", "no"].contains(trimmed),
        "[\(family.id)] thinking: post-</think> output must satisfy the grammar (gate strict); got \(trimmed.debugDescription)"
      )
      print(
        "[grammar-conformance][\(family.id)] thinking: completed — reasoning \(thinkingText.count) chars, "
      ...
      print(
        "[grammar-conformance][\(family.id)] thinking: reasoning surfaced as .thinkingToken "
      ...
      print(
        "[grammar-conformance][\(family.id)] thinking: no .thinkingToken surfaced for this prompt "
  ...
  private func runGemmaCarveOut(_ family: GrammarFamily) async throws {
    precondition(
      !family.expectsGrammarSupport, "runGemmaCarveOut is only for grammar-gated families")
    let backend = try await loadBackend(for: family)
    XCTAssertFalse(
      backend.capabilities.supportsGrammarConstrainedSampling,
