...
final class LlamaArchitecturePreflightTests: XCTestCase {
  ...
  func test_denylist_rejectsClipVisionEncoder() {
    XCTAssertTrue(
      LlamaModelLoader.isUnsupportedArchitecture("clip"),
  ...
  func test_headerReader_extractsArchitectureFromMinimalGGUF() throws {
    ...
    XCTAssertThrowsError(
      try LlamaModelLoader.preflightArchitecture(
  ...
  func test_preflight_acceptsRealChatGGUF() async throws {
    ...
    guard let modelURL = HardwareRequirements.findGGUFModel() else {
      throw XCTSkip(
        "No chat GGUF available. Set LLAMA_TEST_MODEL=<path> or place a `.gguf` in ~/Documents/Models/ to run the preflight happy-path test."
      )
    }
    ...
    do {
      ...
      XCTFail(
        "Real chat GGUF was rejected by the preflight as architecture '\(arch)' — the denylist is too aggressive"
      )
      return
    }
