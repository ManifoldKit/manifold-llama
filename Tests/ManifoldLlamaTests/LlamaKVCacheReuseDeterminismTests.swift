...
final class LlamaKVCacheReuseDeterminismTests: XCTestCase {
  ...
  func test_llama_warmReuseTurnMatchesColdTurnByteForByte() async throws {
    ...
    guard let modelURL = HardwareRequirements.findGGUFModel() else {
      throw XCTSkip(
        "No GGUF model on disk. Set LLAMA_TEST_MODEL or place a .gguf in ~/Documents/Models/.")
    }
    ...
    _ = try await drainEvents(
      try warmBackend.generate(prompt: turn1Prompt, systemPrompt: nil, config: config))
    ...
    let warmEvents = try await drainEvents(
      try warmBackend.generate(prompt: turn2Prompt, systemPrompt: nil, config: config))
    ...
    let coldEvents = try await drainEvents(
      try coldBackend.generate(prompt: turn2Prompt, systemPrompt: nil, config: config))
