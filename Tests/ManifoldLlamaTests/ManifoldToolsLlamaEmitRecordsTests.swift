...
final class ManifoldToolsLlamaEmitRecordsTests: XCTestCase {
  ...
  func test_missingGGUF_emitsNotMeasuredNotFail() throws {
    ...
    XCTAssertEqual(
      result.exitCode, 1,
      "a missing model file is exit 1, not a load failure (3) or a scenario failure")
    XCTAssertTrue(
    ...
    XCTAssertNil(
      record.verdict,
      "an un-measured hole carries no verdict — verdict == .fail would be exactly the false-failure this schema prevents"
    )
  ...
  func test_missingGGUF_emitsHoleForEveryRequestedScenario() throws {
    ...
    let records = try decodeRecords(at: recordsURL)
    XCTAssertEqual(
      records.count, expectedCount, "a hole per scenario in the requested (here: full) corpus")
    XCTAssertTrue(
      records.allSatisfy {
  ...
  func test_realModel_measuredRecordsCarryRealCellIdentity() throws {
    ...
    XCTAssertNotEqual(
      result.exitCode, 3,
      "model load must have succeeded for this assertion to be meaningful: \(result.stderr)")
    XCTAssertTrue(
      FileManager.default.fileExists(atPath: recordsURL.path), "stderr: \(result.stderr)")
    ...
    XCTAssertEqual(record.backend, "llama.cpp")
    XCTAssertEqual(
      record.model, modelURL.lastPathComponent, "must be the real GGUF file name, not a placeholder"
    )
    ...
    XCTAssertEqual(
      record.quant, "Q4_K_M",
      "the fixture's real quant label must survive whole, not truncated to its leading \"Q4\"")
  ...
  private func quantFromMissingModel(named fileName: String) throws -> String {
    ...
    let recordsURL = tmp.appendingPathComponent("records-\(UUID().uuidString).json")
    let transcriptURL = tmp.appendingPathComponent("transcript-\(UUID().uuidString).jsonl")
    let missingModel = tmp.appendingPathComponent(
      "quant-probe-\(UUID().uuidString)", isDirectory: true
    )
  ...
  private func context(transcriptRef: String = "probe") -> ConformanceScorer.RecordContext {
    ConformanceScorer.RecordContext(
      renderer: "jinja-prompt", coreCommit: "test", transcriptRef: transcriptRef)
  }
  ...
  func test_partialTurnWithoutErrorEvent_documentsPreFixDefectShape_andFixRoutesToHole() throws {
    ...
    XCTAssertEqual(
      unfixedRecord.status, .measured,
      "documents the pre-fix defect: no .error event, so the hole is invisible to the scorer")
    XCTAssertEqual(
      unfixedRecord.verdict, .fail,
      "documents the pre-fix defect: a fabricated failure, not a real measurement")
    ...
    XCTAssertNotEqual(
      fixedRecord.status, .measured,
      "the fix must route a mid-stream failure to a hole, never a measured record")
    guard case .loadFail(let reason) = fixedRecord.status else {
      return XCTFail("expected .loadFail, got \(fixedRecord.status)")
    }
    XCTAssertTrue(
      reason.contains("decode failure"), "the real error detail must survive into the hole's reason"
    )
    XCTAssertNil(
      fixedRecord.verdict,
      "a hole carries no verdict — this is what prevents the fabricated .fail above")
  }
}
