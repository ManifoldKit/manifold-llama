...
final class ManifoldToolsLlamaDecoyScoringTests: XCTestCase {
    ...
    private func runCLI(_ arguments: [String]) throws -> (exitCode: Int32, stdout: String, stderr: String) {
        ...
        let cwd = FileManager.default.temporaryDirectory
            .appendingPathComponent("manifold-tools-llama-decoy-tests-\(UUID().uuidString)", isDirectory: true)
    ...
    func test_extraTools_advertisesDecoysWithoutInflatingRequiredTools() throws {
        ...
        // expected-tool set.
        XCTAssertEqual(Set(required), ["now"], "requiredTools must be exactly the scenario's own declared tool — never a decoy name")
        for decoy in decoyNames {
            XCTAssertFalse(required.contains(decoy), "decoy '\(decoy)' leaked into requiredTools — this is exactly the false-negative bug this fix closes")
        }
        ...
        XCTAssertTrue(decoyNames.isSubset(of: Set(advertised)), "all 5 decoys must be advertised to the model even though none are required")
        XCTAssertTrue(advertised.contains("now"), "the real tool must still be advertised alongside the decoys")
        ...
        XCTAssertEqual(scores.recall, 1.0, accuracy: 1e-9, "declining every decoy must not cost recall — the real tool was correctly dispatched")
    }
}
