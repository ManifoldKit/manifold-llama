...
final class RawPromptEvalRunnerTests: XCTestCase {
  ...
  func test_evalRunner_producesWellFormedRawRun() async throws {
    ...
    XCTAssertEqual(
      run.promptSha256, EvalMetadata.sha256Hex(Data(promptText.utf8)),
  ...
  func test_evalRunner_recordsOverriddenTopKAndRepeatPenalty() async throws {
    ...
    XCTAssertEqual(
      run.sampler.repeatPenalty, 1.2,
      "recorded sampler.repeatPenalty must reflect the overridden value actually used")
    XCTAssertFalse(
      run.output.isEmpty, "generation must still produce output with overridden sampler settings")
  }
}
