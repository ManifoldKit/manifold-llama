...
final class LlamaRerankerTests: XCTestCase {
  ...
  func test_rerank_whenNotReady_returnsFirstLimitUnchanged() async throws {
    ...
    XCTAssertEqual(
      result.map(\.chunk.chunkIndex), [0, 1, 2],
  ...
  func test_rerank_withRealModel_ranksRelevantPassageFirst() async throws {
    ...
    guard let rawPath = env["LLAMA_RERANKER_MODEL"], !rawPath.isEmpty else {
      throw XCTSkip(
        "Set LLAMA_RERANKER_MODEL=<path to a cross-encoder reranker GGUF> to run the RANK-pooling end-to-end test."
      )
    }
    ...
    let candidates = [
      VectorSearchHit(
        chunk: DocumentChunk(
          documentID: docID, text: "The Eiffel Tower is located in Paris, France.", chunkIndex: 0),
        documentTitle: "Doc", score: 0.5),
      VectorSearchHit(
        chunk: DocumentChunk(
          documentID: docID,
          text: "Photosynthesis converts sunlight into chemical energy in plants.", chunkIndex: 1),
        documentTitle: "Doc", score: 0.5),
    ]

    let ranked = try await reranker.rerank(
      query: "Where is the Eiffel Tower?", candidates: candidates, limit: 2)

    XCTAssertEqual(ranked.count, 2)
    XCTAssertEqual(
      ranked.first?.chunk.chunkIndex, 0,
...
final class LlamaRerankerErrorPathTests: XCTestCase {
  ...
  func test_rerankerError_modelLoadFailed_includesUnderlyingDescription() {
    ...
    XCTAssertTrue(
      error.errorDescription?.contains("sentinel-message") == true,
...
final class LlamaRerankerLoadCancelTests: XCTestCase {
  ...
  func test_conformsToCancellableModelLoading() {
    let reranker = LlamaReranker()
    XCTAssertNotNil(
      reranker as? any CancellableModelLoading,
  ...
  func test_cancelModelLoad_abortsInFlightLoad_andSettles() async throws {
    ...
    guard let rawPath = env["LLAMA_RERANKER_MODEL"], !rawPath.isEmpty else {
      throw XCTSkip(
        "Set LLAMA_RERANKER_MODEL=<path to a cross-encoder reranker GGUF> to run the in-flight cancel test."
      )
    }
    let modelURL = URL(filePath: (rawPath as NSString).expandingTildeInPath)
    guard FileManager.default.fileExists(atPath: modelURL.path) else {
    ...
    guard reranker.isModelLoadInFlight else {
      ...
      throw XCTSkip(
        "Model loaded before in-flight window was observable — fast-load path, not a failure.")
    }
