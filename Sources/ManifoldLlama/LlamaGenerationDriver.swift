...
@_spi(Testing) public struct LlamaGenerationDriver: LocalInferenceAdapter {
  ...
  func run(
    ...
    prefillHeadroomSampler: @Sendable () -> UInt64? = {
      DeviceCapabilityService.queryAvailableMemory()
    },
    ...
    case .grammarParseFailed:
      ...
      continuation.finish(
        throwing: InferenceError.inferenceFailure("Failed to parse GBNF grammar string"))
    ...
    let thinkingSampler: LlamaSamplerHandle? = {
      ...
      case .chainInitFailed, .grammarParseFailed:
        Self.logger.error(
          "Failed to build thinking-phase sampler; grammar will apply during reasoning")
        return nil
      }
    }()
    ...
    while promptPos < tokens.count {
      ...
      {
        let required =
          footprintEstimator.predictedTransientBytes(
            forTokens: chunkSize,
            safetyFactor: prefillSafetyFactor
          ) ?? remaining
        Self.logger.error(
    ...
    if useToolParser {
      stages.append(
        .tool(
          ToolCallTransform(markers: LlamaToolMarkers.markers(), surfaceTruncatedToolBody: true)))
    }
  ...
  @_spi(Testing) public static func tailRepeats(_ window: [String], phraseLen: Int, minRepeats: Int)
    -> Bool
  {
