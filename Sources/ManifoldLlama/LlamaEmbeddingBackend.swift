...
public final class LlamaEmbeddingBackend: EmbeddingBackend, @unchecked Sendable {
  ...
  fileprivate actor Storage {
    ...
    func install(
      model: OpaquePointer, context: OpaquePointer, vocab: OpaquePointer, dimensions: Int,
      contextSize: Int32, pooling: llama_pooling_type
    ) {
      self.model = model
        ...
        guard contributing > 0 else {
          throw EmbeddingError.encodingFailed(
            underlying: NSError(
              ...
              code: -10,
              userInfo: [
                NSLocalizedDescriptionKey: "llama_get_embeddings_ith returned NULL for every token"
              ]
            ))
        }
        ...
        guard let row = llama_get_embeddings_seq(context, 0) else {
          ...
          throw EmbeddingError.encodingFailed(
            underlying: NSError(
              ...
              userInfo: [
                NSLocalizedDescriptionKey:
                  "llama_get_embeddings_seq returned NULL (pooling=\(pooling.rawValue))"
              ]
            ))
        }
        let buf = UnsafeBufferPointer(start: row, count: dim)
  ...
  public func loadModel(from url: URL) async throws {
    ...
    Self.logger.info(
      "LlamaEmbeddingBackend loaded \(url.lastPathComponent) (dim=\(loaded.dimensions), pooling=\(loaded.poolingType.rawValue))"
    )
  }
    ...
    guard let rawModel = llama_model_load_from_file(url.path, modelParams) else {
      throw InferenceError.modelLoadFailed(
        underlying: NSError(
          ...
          code: -1,
          userInfo: [
            NSLocalizedDescriptionKey: "Failed to load embedding GGUF from \(url.lastPathComponent)"
          ]
        ))
    }
    ...
    guard let ctx = llama_init_from_model(rawModel, ctxParams) else {
      ...
      throw InferenceError.modelLoadFailed(
        underlying: NSError(
          ...
          userInfo: [
            NSLocalizedDescriptionKey:
              "Failed to create embedding context for \(url.lastPathComponent)"
          ]
        ))
    }
